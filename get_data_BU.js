...
function loopForever() {
  ...
  setInterval(function() {
    ...
    function getInformations() {
      ...
      if (currentlyPlayingType != "ad") {
        ...
        if ($("#song-title").text() == "<?=defaultTitleSong; ?>" || response["item"].id != idSong) {
          ...
          //https://www.wikidata.org/w/api.php?action=wbgetentities&format=xml&props=sitelinks&ids=Q271308&sitefilter=frwiki
          ...
          fetch(mbURL1).then(response => { // fetch 1
            ...
            // the issue is that I don't get the correct recording result here
            //http: //musicbrainz.org/ws/2/recording/?query=arid:b7ffd2af-418f-4be2-bdd1-22f8b48613da+AND+recording:Hurt+AND+status:official+AND+tracks:14&fmt=json
            var url2 = `http://musicbrainz.org/ws/2/release-group/?query=release:${cleanAlbumURI}+AND+arid:${mbArtistID}+AND+primarytype:${release_type}+AND+status:official&fmt=json`; //?inc=url-rels&fmt=json/
            console.log(url2)
            ...
            const tmpl = (creditNames, creditAttributes, creditTypes, creditURIs) => `
              <h4>Song Credits</h4>
              <table><thead>
              <tr><th>Artist<th>Contribution<th>Instrument<tbody>
          ...
          fetch(url)
            ...
            .then(function(json) {
              ...
              var wikiUrl = encodeURI(wikiTitle);
              //console.log(wikiUrl);
          ...
          // Wikipedia Search 3: Search for artist data on wikipedia
          var str2 = " (artist)"; // depending on how this string is appended, different results appear. "arist", "music", or "band" gives slightly different results, none perfect.
          var res3 = str3.concat(str2);
          //console.log(res3);
          var wikiSearchArtist = encodeURI(res3);

          //console.log("This should be the ARTIST search string Wikipedia: " + wikiSearchArtist);
          var urlSearchArtist = `https://en.wikipedia.org/w/api.php?action=query&list=search&prop=info&inprop=url&utf8=&format=json&origin=*&srlimit=3&srsearch=${wikiSearchArtist}&origin=*`;
          //console.log("This should be ARTIST search URL to Wikipedia: " + urlSearchArtist);
          fetch(urlSearchArtist)
            .then((response) => {
              return response.json();
            })
            .then((json) => {
              ...
              fetch(url2)
                ...
                .then(data => {
                  ...
                  var myJSON = JSON.stringify(data["query"]["pages"][wikiPageId].extract);
                  var myJSONparsed = JSON.parse(myJSON)
                  document.getElementById("artistInfo").innerHTML = myJSONparsed;
                  //return data;
                })
                .catch(e => {
                  //console.log(e);
                  return e;
                  var noArtistFound = ("Artist Info Found, " + "<a href=\"\r\n" + GoogleLyricsSearch + "\" target=\"_blank\"  >search search again here.<\/a>");
                  //https://www.google.com/search?q=HMLTD+-+To+the+Door+Lyrics
                  document.getElementById("song-lyrics").innerHTML = noArtistFound;
                });
            });
    ...
    if ($("#song-title").text() == "<?=defaultTitleSong; ?>" || response["item"].id != idSong) {
      ...
      /////*[@id="centerContent"]/div[1]/div[4]/div[1]/div[2]/span
